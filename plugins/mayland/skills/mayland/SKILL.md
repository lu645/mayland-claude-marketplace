...
`get_mayledit_capabilities` (1.10.0), then call `list_mayledit_library`,
...
13. When Mayland needs a verified local logo or other local Brand image that it cannot fetch directly, issue the Brand asset handoff, run `mayland-upload "<uploadUrl>" "/absolute/path/to/file"` exactly once, then create the Brand asset with the same Brand and role. Verify the saved asset with `get_brand` or `list_assets`. Mayland never asks the user to authenticate, upload manually, or expose the token.
    Type every image by motif when uploading: `LOGO` and `WORDMARK` for the marks, `PRODUCT` for the packshot, `HERO_SCENE` for atmospheric scenes with room for type, `CUTOUT` for background-free product art, `HAND` for hand interaction shots, `DETAIL` for close-ups, `VARIANT` for colourway sets. Motif roles carry the `productId` they belong to. Shop galleries mix clean photos with marketing creatives that carry burned-in headlines and badges; look at every image before uploading it and skip the creatives.
