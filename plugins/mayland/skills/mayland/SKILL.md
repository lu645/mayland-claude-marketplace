...
4. Before writing, call `list_brands`, then `get_brand` when the active Brand exists. Match its website and identity and pass the returned brandId. Equivalent website spellings must not create a second active Brand. Archived Brands do not block creating a new Brand with the same name or website. Multiple active matches need clarification and an explicit returned brandId. An archived Brand must not be revived silently: the user can restore it explicitly from Campaigns > Archived brands (the trash icon). Keep existing values, especially manual fields. Missing fields may be filled; conflicts are reported instead of overwritten.
   - Resolve errors using the advertised MCP tools and the returned recovery guidance. Do not assume access to Mayland's source code, database or server, and do not ask ordinary users to inspect them. A deterministic identity error needs corrected input or a user choice, not repeated identical calls. Reuse the original key for an uncertain transport result; use a new key when correcting the input.
5. Build the verified Brand payload:
   ...
   - `category` is exactly one of these keys, never free text: `apparel_footwear` (Apparel & Footwear), `baby_kids` (Baby, Kids & Toys), `bags_accessories` (Bags & Accessories), `beauty_cosmetics` (Beauty & Cosmetics), `beverages` (Beverages), `electronics` (Electronics & Gadgets), `food_snacks` (Food & Snacks), `health_supplements` (Health & Supplements), `hobbies_media` (Hobbies, Books & Media), `home_garden` (Home & Garden), `jewelry_watches` (Jewelry & Watches), `personal_care` (Personal Care & Hygiene), `pet_supplies` (Pet Supplies), `sports_outdoors` (Sports & Outdoors), `other` (Other). Pick the bucket of the Brand's main product range and `other` only when none fits; describe the range itself in `truth` or `notes`. Any other value is refused with BRAND_CATEGORY_INVALID.
   ...
   - Inspect visible announcement bars, scrolling marquees, promotional strips, buttons and large accent surfaces in addition to logos and neutral backgrounds. Record each verified color's HEX, role, source URL and retrieval date in notes; use actual CSS/computed styles or sampled reference pixels, never a transcript's color word to invent a HEX. Include defining accents in palette. If an existing palette is manually maintained, preserve it and store the evidenced additional colors and conflicts in notes/openPoints for explicit reconciliation; do not silently replace it. For the SNOCKS regression, verify the official site's announcement/marquee reference and record its evidence, or explicitly report that the historical yellow could not be verified.
   - Store multiple verified font families as `designDna.font_families`: entries have `name`, `usage` (e.g. body, headlines, campaign accent), optional HTTPS font `data` source and real `weight` (100–900). Use one entry per static weight; retain existing displayFont compatibility. Do not replace existing manual families. Missing or inaccessible font files are open points.
   - Fill `intelligence`, the material a first email is built from; every entry carries the sourceUrl that states it. `rating` (score, scale, count, source) from JSON-LD AggregateRating, the shop's review widget or public review pages; up to 30 verbatim `reviews` with the name as published, never paraphrased or invented; `trustFacts` (shipping threshold, delivery time, returns, guarantee, payment, press, origin) from service, FAQ and shipping pages; `story` (founders, since, origin, mission, quote) from the about page; `ctaPool` with button labels the brand already uses; `voiceSamples` with sentences the brand wrote itself; `painPoints` customers name in reviews, ads or product pages.
   - Measure the design signature on the rendered site instead of estimating it: designDna `type_scale` (display, h1, h2, h3, body in px at a 600px email width, scaled from the site's hero-to-body ratio), `button_radius`, `panel` (radius, border_width, border_color, shadow of cards), `color_roles` (background, surface, band, cta, ink, highlight from computed styles, with source_url) and `signature_motifs` (recurring patterns, shapes, stickers or illustrations, their usage and, once uploaded, their asset_id).
