...
- One-time transition: the first update from an older release-scoped plugin moves the connection to a release-independent server address, so that single update asks for one final browser re-authorization; afterwards the address is stable and future plugin updates keep the existing connection.
