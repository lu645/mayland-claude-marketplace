...
- A bare MCP reconnect or CLI version report does not verify a plugin update. Do not substitute separate update/login commands for the verified setup when installing or updating the plugin. An already verified installation may start a fresh OAuth login to renew authorization after explicit browser confirmation.
...
- If MCP reports `TOKEN_FAMILY_REVOKED` or `TOKEN_REUSE`, stop retries and preserve the unfinished request without credentials. Close older Claude sessions using the same Mayland connection, then obtain a NEW command from Connect Agent with Claude selected and run it once. Start a fresh Claude session after successful setup so the old in-memory credentials are not reused. Do not edit Claude's credential cache or reuse the failed OAuth URL. Read back the affected product/import state before retrying a write, keeping its original idempotency key and unchanged payload when retrying the same request.
...
- The launcher requests opening the emitted OAuth URL with `open` on macOS or `xdg-open`/`wslview` on Linux or WSL. If opening fails or no tab appears, give the user the printed Mayland `/api/oauth/authorize` URL to open in a browser. Keep the setup command running and do not start a second login. Mayland may ask the user to sign in and explicitly confirm the workspace before redirecting to the waiting localhost callback.
...
- If Python 3 is unavailable or the pseudo-terminal cannot start, report the exact failure. If browser confirmation expires, start a fresh OAuth login after the old listener stops. Do not switch to an unqualified server name.
