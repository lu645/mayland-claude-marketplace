...
- If MCP reports `TOKEN_FAMILY_REVOKED` or `TOKEN_REUSE`, stop retries and preserve the unfinished request without credentials. Close older Claude sessions using the same Mayland connection, then obtain a NEW command from Connect Agent with Claude selected and run it once. Start a fresh Claude session after successful setup so the old in-memory credentials are not reused. Do not edit Claude's credential cache, reuse the failed OAuth URL, or retry a bare MCP login: the original handoff was consumed or expired. Read back the affected product/import state before retrying a write, keeping its original idempotency key and unchanged payload when retrying the same request.
