...
- ALWAYS run that complete command unchanged, even when MCP already reports Connected. It refreshes the marketplace and plugin, verifies the exact installed version, package contents and deployment, enables Mayland automatic updates, and only then starts OAuth.
...
- The launcher automatically opens the emitted OAuth URL with `open` on macOS or `xdg-open`/`wslview` on Linux or WSL. If the wrong browser opens, use the printed Mayland `/api/oauth/authorize` URL in the browser already signed in to Mayland. Keep the setup command running and do not start a second login; that browser redirects the private connect intent back to the waiting localhost callback.
